...
**Status:** 🟢 Active - Production Features Ready
...
- [✅] Created user_prompt_preferences database table
- [✅] Implemented server actions for per-prompt storage (lib/prompts/copy-preferences.ts)
- [✅] Updated CopyButton to use prompt-specific settings
- [✅] Created CopyPreview component for real-time preview
- [✅] Added Ultrathink and GitHub reminder options
- [✅] Removed GlobalSettings (replaced by per-prompt approach)
- [✅] Settings persist across prompt detail and list views
- [✅] Database storage for logged-in users
- [✅] localStorage with prompt-specific keys for anonymous users

**Authentication & UI Fixes:**
- [✅] Fixed broken /auth/login links (changed to /auth/signin)
- [✅] Added dark mode support to signin/signup pages
- [✅] Migrated from useFormState to useActionState (React 19)
- [✅] Created admin password management scripts
...
- `lib/invites/` - Invite system module (validation, actions, types) ✅
- `scripts/create-first-admin.ts` - Bootstrap script ✅
- `prisma/schema.prisma` - invite_codes table ✅
...
- Invite chain tracking enables moderation (ban bad actor → see who they invited)
...
**Per-Prompt Copy Preferences System:**
- ✅ Database architecture: Created `user_prompt_preferences` table with unique constraint on (user_id, prompt_id)
- ✅ Server actions: `getPromptCopyPreferences()` and `savePromptCopyPreferences()` with upsert pattern
- ✅ CopyButton: Refactored to use prompt-specific localStorage keys and database storage
- ✅ CopyPreview: New component showing real-time preview of final copy output
- ✅ Settings persistence: Different settings per prompt, synced across pages
- ✅ Storage strategy: Database for logged-in users, localStorage for anonymous
- ✅ New options: Ultrathink and GitHub reminder checkboxes

**Authentication & UI Fixes:**
- ✅ Fixed 404 on /auth/login (updated to /auth/signin across 3 files)
- ✅ Dark mode for signin/signup pages (comprehensive Tailwind classes)
- ✅ React 19 migration: useFormState → useActionState
- ✅ Admin scripts: check-admins.ts and reset-admin-password.ts

**Database Migrations:**
- ✅ Migration 1: Add copy preferences to users table (prefix, suffix, booleans)
- ✅ Migration 2: Add ultrathink and GitHub reminder fields
- ✅ Migration 3: Create user_prompt_preferences table (per-prompt storage)
...
- Per-prompt copy preferences fully implemented ✅
